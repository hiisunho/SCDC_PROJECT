...
<body>
	<div id="wrapper">
        ...
        <div id="page-wrapper" style="min-height: 868px;">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header" align="center">출고 요청</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                            	...
                            	<table width="100%" class="table table-striped table-bordered table-hover">
                                	...
                                	<tbody>
				                        ...
				                        <c:forEach var="request" items="${request}">                              
		                                    <tr class="odd gradeX">
		                                        <td style="text-align:center"><input type="checkbox" name="supportNo" value="${request.supportNo}"></td> 
		                                        <td><c:out value="${request.supportNo}"></c:out></td>                               
		                                        <td><c:out value="${request.partCode}"></c:out></td>
		                                        <td><c:out value="${request.materialProcess}"></c:out></td>
		                                        <td><fmt:formatDate pattern="yy-MM-dd" value="${request.daySchedule}"/></td>
		                                        <td><c:out value="${request.requirement}"></c:out></td>
		                                        <td><c:out value="${request.stockQuantity}"></c:out></td>  
		                                    </tr>
				                        </c:forEach>                                  
                                    </tbody>
	                            </table>
									...
									<div class="col-lg-10">
			                        	<div class="form-group">
					                         <c:if test = "${pageMaker.prev}">
					                            	<a href="/export/request?pageNum=${pageMaker.startPage-1 }&amount=${pageMaker.cri.amount}"> prev </a>
					                            </c:if>
					
					                            <!-- num는 페이지 영역 -->
					                           <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="num">
					                            	&nbsp;&nbsp; 
					                            	<a href="/export/request?pageNum=${num}&amount=${pageMaker.cri.amount}">${num}</a> 
					                            	&nbsp;&nbsp;&nbsp;
					                            	
					                            </c:forEach>
					                            
					                            <c:if test = "${pageMaker.next}">
					                            	<a href="/export/request?pageNum=${pageMaker.endPage+1 }&amount=${pageMaker.cri.amount}"> next </a>
					                            </c:if>
					                      </div>
			                 		</div>
			                 		...
			                 		<div class="col-lg-1">
			                        	<div class="form-group">
			                          		<input type="submit" id="btn" class="btn btn-primary" value="요청하기" style="float:right">
			                     		</div>
			                 		</div>
...
<script>
	$("#btn").click(function() {
		if ($("input:checkbox[name='supportNo']").is(":checked")==false) {
